...
def resultProcess(file_name):
    ...
    with open(file_name + '.csv', 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            cates = row[5].split('    ')
            for cate in cates:
                if re.search("[验]", cate.strip()) and re.search("[经]", cate.strip()):
                    model["经验要求"]=cate
                elif re.search("[本科博士大专硕士高中]{2,}", cate.strip()):
                    model["学历要求"] = cate
                elif re.search("[招]", cate.strip()) and re.search("[人]", cate.strip()):
                    model["招聘人数"] = cate
                elif re.search("[发布]", cate.strip()):
                    model["发布时间"] = cate
                elif re.search("[良好练精]", cate.strip()) and re.search("[语话]", cate.strip()):
                    model["口语要求"] = cate
